...
import {DashboardHarness} from 'src/app/dashboard_view/dashboard.harness';
...
import {initialState, mockProductResponse} from 'src/app/shared/mock/test_mocks';
...
import {isLoading} from 'src/app/store/product/product.selectors';
import {getProductsAction} from '../store/product/actions/get-products.actions';
...
describe('Dashboard', () => {
  ...
  const mockChartProductData = {
    fieldNames: ['Task', 'Hours per Day'], data: [
  ...
  beforeEach(async () => {
    await TestBed.configureTestingModule({
      imports     : [NoopAnimationsModule, RouterTestingModule], declarations: [
        TestComponent,
        DashboardComponent,
    ...
    component = fixture.debugElement.query(
      By.directive(DashboardComponent)).componentInstance;
  ...
  it('should dispatch products retrieval OnInit creation', () => {
    ...
    expect(store.dispatch).toHaveBeenCalledWith(getProductsAction({
      lang: initialState.lang, pageIndex: 0,
    }));
  });
  ...
  describe('Pie Chart', () => {
    ...
    it('should pass product name as a title in chartConfig', () => {
      expect(pieChartComponent.chartConfigs.title)
        .toContain(targetProduct.name);
    });
  });
  ...
  describe('Bar Chart', () => {
    ...
    it('should pass product name as a title in chartConfig', () => {
      expect(barChartComponent.chartConfigs.title)
        .toContain(targetProduct.name);
    });
  });
});
